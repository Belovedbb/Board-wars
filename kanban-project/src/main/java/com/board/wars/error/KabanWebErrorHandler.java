...
@Component
@Order(-2)
public class KabanWebErrorHandler implements WebExceptionHandler {

    @Override
    public Mono<Void> handle(ServerWebExchange exchange, Throwable exception) {
        if(! (exception instanceof KanbanResponseException)){
            exception.printStackTrace();
            exception = transformToKanbanResponseException(exception);
        }
        KanbanResponseException error = (KanbanResponseException) exception;
        return handleKanbanResponseException(exchange, error);
    }
    
    private Mono<Void> handleKanbanResponseException(ServerWebExchange webExchange, KanbanResponseException error){
        return Mono.just(webExchange)
                .map(resp -> setMetaData(resp, error))
                .flatMap(exchange -> setBody(exchange, error.getBody()));
    }
    
    private ServerWebExchange setMetaData(ServerWebExchange exchange, final KanbanResponseException error){
    ...
    private KanbanResponseException transformToKanbanResponseException(Throwable exception){
        return new Utilities.KanbanErrorBuilder()
