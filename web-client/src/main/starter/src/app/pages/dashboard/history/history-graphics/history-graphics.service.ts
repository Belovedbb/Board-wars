...
export class HistoryGraphicsService {
  constructor(private messageService: MessageService, private http: ApiHttpService) { }

  registerGraphicsEvent(data: Array<TimeLineSnippet>) : Subscription {
    return this.observeMessages(Config.API_ROUTE.HISTORY_EVENT_KANBAN+"/graphics")
  ...
  private  transformHistoryToTimeLineSnippet(history: History): TimeLineSnippet {
    ...
    timeline.footer = 'Graphics Event';
  ...
  getPersistentTimelineData(page: number, size: number): Observable<TimeLineSnippet[]> {
    return this.http.get(Config.resolveParameterLink(Config.API_ROUTE.HISTORY_PERSISTENT_KANBAN, "graphics", ""+page, ""+size) )
      .pipe(
