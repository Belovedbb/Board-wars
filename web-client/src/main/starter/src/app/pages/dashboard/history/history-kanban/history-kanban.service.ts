...
export class HistoryKanbanService {
  constructor(private messageService: MessageService, private http: ApiHttpService) { }

  registerKanbanEvent(data: Array<TimeLineSnippet>) : Subscription {
    return this.observeMessages(Config.API_ROUTE.HISTORY_EVENT_KANBAN+ "/project")
  ...
  getPersistentTimelineData(page: number, size: number): Observable<TimeLineSnippet[]> {
    return this.http.get(Config.resolveParameterLink(Config.API_ROUTE.HISTORY_PERSISTENT_KANBAN, "domain", ""+page, ""+size) )
      .pipe(
