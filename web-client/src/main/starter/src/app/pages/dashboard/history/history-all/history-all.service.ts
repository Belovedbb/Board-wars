...
export class HistoryAllService {
  constructor(private messageService: MessageService, private http: ApiHttpService) { }

  registerAllEvent(data: Array<TimeLineSnippet>) : Subscription {
    return this.observeMessages(Config.API_ROUTE.HISTORY_EVENT_ALL)
  ...
  getPersistentTimelineData(page: number, size: number): Observable<TimeLineSnippet[]> {
    return this.http.get(Config.resolveParameterLink(Config.API_ROUTE.HISTORY_PERSISTENT_ALL, ""+page, ""+size) )
      .pipe(
